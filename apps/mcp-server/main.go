...
type CheckInput struct {
	...
	Type     string `json:"type,omitempty" jsonschema:"forced type: json, xml, yaml, toml, ini, csv, tsv, hcl, markdown, env, properties, sql:mysql, sql:postgres, sql:ansi, sql:sqlite, sql:mssql, sql:oracle; empty means auto-detect from the extension"`
...
func main() {
	...
	mcp.AddTool(server, &mcp.Tool{
		Name:        "check_syntax",
		Description: "Validate the syntax of a file (JSON, XML, YAML, TOML, INI, CSV/TSV, HCL, Markdown, .env, Properties, or SQL). Optionally validate JSON or YAML against a JSON Schema by passing the 'schema' path. Returns whether the file is valid and the list of syntax errors with line/column when available.",
	}, checkSyntax)
